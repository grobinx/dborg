    ...
    return (
        <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={minimized ? { bottom: 0, top: 0, left: 0, right: 0 } : { bottom: 30, top: 5, left: 0, right: 0 }}>
                ...
                {!minimized && <YAxis yAxisId="right" orientation="right" stroke={theme.palette.info.main} style={{ fontSize: "0.75rem" }} tickFormatter={prettySize} label={{ value: t("buffers-alloc", "Buffers Alloc"), angle: -90, position: "insideRight", fill: theme.palette.info.main, fontSize: 11 }} />}
                ...
                <Area type="monotone" dataKey="buffers_alloc" yAxisId={minimized ? undefined : "right"} stroke={theme.palette.info.main} fillOpacity={1} fill="url(#colorBufAlloc)" name={t("buffers-alloc", "Buffers Alloc")} isAnimationActive={false} connectNulls dot={false} />
            </AreaChart>
        </ResponsiveContainer>
    );
};