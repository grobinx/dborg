    ...
    "Progress i alerty": [
        { type: "progress", label: "Indeksowanie", value: 18, showPercent: true, severity: "info" },
        {
            ...
            refresh: {
                ...
                label: "Przetwarzanie",
                value: async (data) => {
                    if (data._lastValue === 100) {
                        data._lastValue = 0;
                    } else {
                        data._lastValue = (data._lastValue ?? 0) + Math.random() * 20;
                        if (data._lastValue > 100) data._lastValue = 100;
                    }
                    return Math.round(data._lastValue);
                },
                bufferValue: async (data) => Math.min(100, (data._lastValue ?? 0) + Math.random() * 30),
                showPercent: true,
                severity: "warning"
            }
        },
